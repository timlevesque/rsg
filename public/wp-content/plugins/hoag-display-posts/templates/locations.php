...
<?php // print $original_atts['tax_term'];?>
	...
	<div class="card h-100">
		<figure>
			<a class="entry-image-link" href="<?php echo get_permalink(); ?>"><?php echo get_the_post_thumbnail( get_the_ID(),'thumbnail', array('class' => 'img-fluid')); ?></a>
		</figure>
		<h5 class="card-title"><?php if ($display_name == "") /* the fail safe ---->*/ {print $local_title;} else{print $display_name;}?></h5>
		<?php if($alert  !=''):?>
		...
		<?php if (isset($original_atts['tax_term']) && $original_atts['tax_term'] != 'urgent-care'):  ?>
			<div class="card-body row pt-5 px-5 m-0 mx-0">
				<?php if ($office_name !=''):?>
