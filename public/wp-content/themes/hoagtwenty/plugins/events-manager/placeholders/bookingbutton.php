...
if( is_user_logged_in() ){ //only show this to logged in users
	...
	if( is_object($EM_Booking) && $EM_Booking->booking_status != 3 && get_option('dbem_bookings_user_cancellation') ){
		?><a id="em-cancel-button_<?php echo $EM_Booking->booking_id; ?>_<?php echo wp_create_nonce('booking_cancel'); ?>" class="px-4 py-2 btn bg-tertiary em-cancel-button text-sm-sm" href="#"><?php echo $button_cancel; ?>&nbsp;&#8722;</a><?php
	}elseif( $EM_Event->get_bookings()->is_open() ){
		if( !is_object($EM_Booking) ){
			?><a id="em-booking-button_<?php echo $EM_Event->event_id; ?>_<?php echo wp_create_nonce('booking_add_one'); ?>" class="px-4 py-2 btn btn-primary em-booking-button text-sm-sm" href="#"><?php echo $button_text; ?>&nbsp;&#43;</a><?php 
		}else{
		...
		?><span class="em-closed-button text-muted"><?php echo $button_closed ?></span><?php
	}
